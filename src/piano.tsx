...
Remember: You are ACTIVELY MONITORING their progress. Describe what you observe before giving the next instruction!`;
  ...
  const stopStreaming = async () => {
    ...
    if (onEndSession) {
      ...
      try {
        ...
        setStatusMessage(`${randomAvatar.name} is writing your session summary...`);
        
        console.log('📝 Generating session summary with avatar:', randomAvatar.name);
        const summary = await generateSessionSummary(messages, sessionDuration, randomAvatar);
  ...
  const generateSessionSummary = async (messages: Message[], duration: number, avatar: any): Promise<string> => {
    if (!apiKey) {
...
Please create a concise summary of this piano lesson session.
...
Write the summary in your character's voice and personality, as if you're the instructor reflecting on the lesson. Keep it very short and concise.`;
